        ...
        if chats_list != []:
            for chat in chats_list:
                ...
                if not m_id in list_message_seened and chat['object_guid'] in admins:
                    ...
                    if text == "شروع" :
                        bot.sendMessage(chat['object_guid'],'♦️ایدی کانال مورد نظر را وارد کنید:👀️')
                    elif text.startswith("@"):
                        id = text.split("@")[1]
                        guid_of_channel = bot.getInfoByUsername(id)['data']['channel']['channel_guid']
                        bot.sendMessage(chat['object_guid'],'♦️کپشن را وارد کنید:📖')
                    elif text.startswith("cp ["):
                        caption = id = text.split("[")[1][:-1]
                        bot.sendMessage(chat['object_guid'],'♦️فرمت فایل را وارد کنید!🗂️')
                    elif text.startswith("-"):
                        new_name = f" {text}"
                        bot.sendMessage(chat['object_guid'],'♦️پست مورد نظر را فوروارد کنید:📥')
                        ...
                        bot.sendMessage(chat['object_guid'],'♦️آپلود شروع شد!📤')
                                ...
                                while True:
                                    try:
                                        ...
                                        if a > tweny_five and is_tweny_five == False:
                                            bot.sendMessage(chat['object_guid'],'♦️25 درصد از آپلود انجام شده...📤')
                                            is_tweny_five = True
                                        elif a > fifty and is_fifty == False:
                                            bot.sendMessage(chat['object_guid'],'♦️50 درصد از آپلود انجام شده...📤')
                                            is_fifty = True
                                        elif a > seventy_five and is_seventy_five == False:
                                            bot.sendMessage(chat['object_guid'],'♦️75 درصد از آپلود انجام شده...📤')
                        ...
                        if j != None and type(j) == str:
                            ...
                            bot.sendMessage(chat['object_guid'],'آپلود نشد‼️')
